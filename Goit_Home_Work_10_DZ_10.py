...
# Примітка : Оскільки клас пустий , то фактично ми придумуємо довільне імя класу,
            ...
            # Схема : # Оголошуємо потрібний нам клас : # class *ЯкесьІмяЯкеОписуєХочЯкосьВиключення(Exception): 
...
while True :  # Бескінечний цикл в яякому будемо просити ввечти id згідно шаблону.
    ...
    try: # Оператор перехоплення винятків/помилок 
        ...
        print(add_id(id_list,employee_id_input)) # Якщо введ правильно, то відразу повернем результат роботи функції *add_id(id_list,employee_id_input)
        ...
        print(f"Невірний шабло , добавте перед вашим id 01") # Принтимо підсказку що пішло не так .
