...
from collections import UserDict # З бібліотеки *collections імпортуємо вбудований клас *UserDict - 
        ...
        for key in self.data: # Цикл в якому будемо проходитись по ключах(*key) з словнику *data # *self.data - це звернення до батьківського поля яке буде містити наш словник з екзепляру субкласу *LookUpKeyDict )
            ...
            if self.data[key] == value: # Умова, якщо значення поточного ключа *key з словника *data,
             ...
             keys.append(key)           # Якщо умова виконалась, з начить поточний ключ *key містить задане значення *value
                                        # Вбудованим методом *.append(*якийсь_ключ) добавляємо його в нащ список *keys
...
# Наповняємо *some_dict - наш екзепляр(словник) субкласу *LookUpKeyDict - значеннями,
...
# про те він підтримує і наш власний метод *.lookup_key(*arg) якого немає в жодному іншому словнику. 
#  Примітка : що робит власний  метод *.lookup_key(*arg) класу *LookUpKeyDict дивись в реалізації цього методу в даному класі в коді вище
...
print(some_dict.lookup_key(25))  # Принтемо результ виконня методу з заданим аргументом 20 .
