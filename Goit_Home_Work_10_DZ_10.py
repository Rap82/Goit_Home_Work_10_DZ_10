...
# ================================   КОНТАКТНА КНИГА, ЧАСТИНА Третя(остання)  =======

# ++++++++++++++++++++++++++++++++++++++ Умова / Condition ++++++++++++++++++++++++++++++++++++++++

# Завершуємо функціональність класу Contacts. На цьому етапі ми додамо до класу метод remove_contacts. 
# Метод винен видаляти контакт по унікальному id у списку contacts. Якщо словника із зазначеним id у списку contacts не знайдено, метод жодних дій над списком contacts не робить.

# Примітка: для правильного проходження тесту не створюйте екземпляр класу в коді.

# ++++++++++++++++++++++++++++++++++++++ Код / Code ++++++++++++++++++++++++++++++++++++

# Примітка : Робить все тесаме що й завданні 15 . Просто добавляється ще один власний метод *.remove_contacts(*якесь_id)
             # Логіка пошуку потрібного словника аналогічно до *.get_contact_by_id(*якесь_id) єдине.
             # що ми знайдений елемент списку який відповідає умові.  не повертаємо а видаляємо з нашого списку *self.contacts
             # Для видалення застосовуємо стандартний метод списку *.remove(*елемент_списку_який_видаляємо)
             # Після видалення, повертаємо відразу список *self.contacts з методу 
class Contacts:
    current_id = 1
